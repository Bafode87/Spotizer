<template>
    <section class="list">
        <div class="list__items" v-for="item in props.songs" :key="item.id">
            <div class="item">
                ...
                <button v-if="props.removeSong" @click="$emit('removeSong', item.id)" class="item__remove">
                    <svg width="36" height="36" class="removeSong">
                        <use xlink:href="#removeSong"/>
                    </svg>
                </button>
            </div>
        </div>
    ...
    <symbol style="display: none;">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 96 960 960" fill="red" id="removeSong">
